...
//  private double[] inputData;
     ...
     * 20 mel-filters are place in the range from 20 to 8000 Hz.
     *
    ...
    {
        this(sampleRate, 512, 60, true, 20.0, 8000.0, 20);
    }


    /**
     * Creates a new MFCC object. 20 mel-filters are place in the range from 20 to
     * 8000 Hz.
     *
     * @param sampleRate float samples per second, must be greater than zero; not
     *                         whole-numbered values get rounded
    ...
    {
        this(sampleRate, windowSize, numberCoefficients, useFirstCoefficient, 20.0, 8000.0, 20);
    }
...
//    inputData = new double[windowSize];
     ...
     *
     * @param inputData double[] input data is an array of samples, must be a multiple
     ...
     * @throws IllegalArgumentException raised if method contract is violated
     */
    public double[][] process(double[] inputData) throws IllegalArgumentException, IOException
    {
        double[] input;
        //check for null
        if(inputData == null)
        ...
        int padding = hopSize - len % hopSize;
        input = new double[len + padding];
        for (int i = 0; i < len; i++) {
            input[i] = inputData[i];
        }
